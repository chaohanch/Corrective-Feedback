...
# source for dataset
source("prepare_data_for_modeling_wDelay.R")
...
df_subjdiff <- df_diffmod %>%
  # get mean diff socre
  group_by(TrainingSession, ID, Group) %>%
  dplyr::summarize(mean_posVSpre = mean(pos_minus_pre),
                   mean_delVSpre = mean(del_minus_pre)) %>%
  ungroup()
...
df_all4cor_outlier <- df_all4cor %>%
  ...
  # identify mean_posVSpre outliers
  mutate(rating_posVSpre_upThres = mean(mean_posVSpre, na.rm = TRUE) + out_thres*sd(mean_posVSpre, na.rm = TRUE),
         rating_posVSpre_lowThres = mean(mean_posVSpre, na.rm = TRUE) - out_thres*sd(mean_posVSpre, na.rm = TRUE),
         rating_posVSpre_outliers = mean_posVSpre > rating_posVSpre_upThres | mean_posVSpre < rating_posVSpre_lowThres) %>%
  # identify mean_delVSpre outliers
  mutate(rating_delVSpre_upThres = mean(mean_delVSpre, na.rm = TRUE) + out_thres*sd(mean_delVSpre, na.rm = TRUE),
         rating_delVSpre_lowThres = mean(mean_delVSpre, na.rm = TRUE) - out_thres*sd(mean_delVSpre, na.rm = TRUE),
         rating_delVSpre_outliers = mean_delVSpre > rating_delVSpre_upThres | mean_delVSpre < rating_delVSpre_lowThres) %>%
...
# correction before removing outliers
df_cor <- df_all4cor %>%
  select(TrainingSession:ID, WM_n_correctRecall, mean_posVSpre)

cor.test(df_cor$WM_n_correctRecall, df_cor$mean_posVSpre)
...
df_cor <- df_all4cor_outlier %>%
  select(TrainingSession:ID, WM_n_correctRecall, mean_posVSpre,
         WM_outliers, rating_posVSpre_outliers) %>%
  # remove outlier
  filter(WM_outliers != TRUE & rating_posVSpre_outliers != TRUE)
...
# plotting ####
ggplot(data = df_cor, 
       mapping = aes(x = WM_n_correctRecall, y = mean_posVSpre)) +
  geom_point() +
  scale_colour_Publication() +
  scale_fill_Publication() +
  theme_Publication() +
...
# label ~ pronounce
df_corrmatrix <- df_all4cor %>%
  select(c(Group, ID:NWR_n_correctRecall, mean_pre, mean_posVSpre:mean_delVSpre))

res2 <- rcorr(as.matrix(df_corrmatrix[df_corrmatrix$Group == "meta", 3:ncol(df_corrmatrix)]))
flattenCorrMatrix(res2$r, round(res2$P, 4))
